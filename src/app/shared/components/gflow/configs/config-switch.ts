...
@Component({
    ...
    imports: [FormsModule, ButtonModule, InputTextModule, TextFieldComponent],
    template: `
        <div class="config-fields">
            <app-text-field
                label="Champ à évaluer"
                placeholder="ex: data.type"
                [(ngModel)]="field"
            ...
            <div class="config-field">
                ...
                <div class="cases-list">
                    @for (caseItem of cases; track $index) {
                        <div class="case-item">
                            <input pInputText
                                [(ngModel)]="caseItem.label"
                                placeholder="Label"
                                pSize="small"
                                class="case-input"
                                (ngModelChange)="onCaseChange($index)" />
                            <input pInputText
                                [(ngModel)]="caseItem.value"
                                placeholder="Valeur"
                                pSize="small"
                                class="case-input"
                                (ngModelChange)="onCaseChange($index)" />
...
export class ConfigSwitchComponent {
    ...
    get config(): SwitchConfig {
        return this.node().config as SwitchConfig;
    }
    ...
    get cases(): SwitchCase[] {
        if (!this.config.cases) {
            this.config.cases = [{ label: 'Case 1', value: '' }];
        }
        return this.config.cases;
    }

    onChange(): void {
        this.configChange.emit({ type: 'config-updated', config: this.config });
    }
