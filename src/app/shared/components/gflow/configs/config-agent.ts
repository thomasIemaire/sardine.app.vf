            ...
            @if (selectedAgent) {
                <div class="agent-info">
                    <div class="agent-info__row">
                        <span class="agent-info__label">ID:</span>
                        <span class="agent-info__value">{{ selectedAgent.id }}</span>
                    </div>
                    <div class="agent-info__row">
...
export class ConfigAgentComponent implements OnInit {
    ...
    availableAgents: AgentOption[] = [
        { id: 'agent-1', name: 'Agent Extraction', version: '1.0.0' },
        { id: 'agent-2', name: 'Agent Validation', version: '2.1.0' },
        { id: 'agent-3', name: 'Agent Notification', version: '1.5.0' },
    ];

    selectedAgent: AgentOption | null = null;

    ngOnInit(): void {
        const config = this.node().config as AgentConfig;
        if (config.agentId) {
            this.selectedAgent = this.availableAgents.find(a => a.id === config.agentId) || null;
        }
    }

    get config(): AgentConfig {
        return this.node().config as AgentConfig;
    }
